...
_maxbackups=3
_backuploc=./backups
_dependencies=YES	#currently compatible with Ubuntu 15 & 16
...
function fDependencies(){
	...
	if [ $OS = "Ubuntu" ] && [ $(dpkg-query -W -f='${Status}' lib32gcc1 2>/dev/null | grep -c "ok installed") -eq 0 ]; then
                        if [ "$EUID" -ne 0 ]; then
       			...
       			apt-get -y install lib32gcc1 libc6 gcc-multilib screen
...
function fBuildOxide(){
        ...
        echo ""
        	...
        	wget --waitretry=5 --output-document=Oxide-Rust.zip https://dl.bintray.com/oxidemod/builds/Oxide-Rust.zip --no-check-certificate
        	fi
	mkdir -p ./temp/server/$_ident && cp -R ./rust/server/$_ident/ ./temp/server/
...
function fBackup(){
        if [ -d "./rust/server/"$_ident"/oxide/" ]; then
                        echo ""
